...
namespace fsge {
	...
	void Matrix<T>::multiply(Matrix<T> m) {
		if (m_c != m.get_rows()) {
			// fsgeError_error(fsgeMatrixOperationSizeError, "Multiply", __FILE__, __LINE__);
		}
	...
	void Matrix<T>::add(Matrix<T> m) {
		if (m_c != m.get_cols() || m_l != m.get_rows()) {
			// fsgeError_error(fsgeMatrixOperationSizeError, "Add In", __FILE__, __LINE__);
		}
	...
	Matrix<T> Matrix<T>::translation(Vector<T> v) {
		if (v.get_size() != 3) {
			// fsgeError_error(fsgeMatrixOperationSizeError, "Translation", __FILE__, __LINE__);
		}
